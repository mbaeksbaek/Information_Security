...
gcc -Wall -Wextra -O2 -Iinclude -Itests \
  ...
  tests/kat/rsa_kat_vectors.c \
  tests/test_all_rsa.c \
  -o build/test_all_rsa

실행: ./build/test_all_rsa
*/
...
int main(void)
{
    ...
    printf("[1] bigint unit tests...\n");
    if (test_bigint_main() != 0) fail = 1;

    printf("\n[2] bigint capacity tests...\n");
    if (test_bigint_capacity_main() != 0) fail = 1;

    printf("\n[3] rsa_core extended tests...\n");
    ...
    printf("\n[6] Runner + dummy ops smoke (pipeline sanity)...\n");
    if (test_runner_smoke_main() != 0) fail = 1;

    printf("\n[7] RSA CLI pipeline (enc/dec, line/whole)...\n");

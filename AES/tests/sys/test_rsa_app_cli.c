...
static void test_rsa_cli_line_raw_roundtrip(void) {
    const char *pt_path   = "res/test/runner/tmp_rsa_cli_pt_line.txt";
    const char *ct_path   = "res/test/runner/tmp_rsa_cli_ct_line.bin";
    const char *pt2_path  = "res/test/runner/tmp_rsa_cli_pt_line_dec.txt";
...
static void test_rsa_cli_whole_raw_roundtrip(void) {
    const char *pt_path   = "res/test/runner/tmp_rsa_cli_pt_whole.txt";
    const char *ct_path   = "res/test/runner/tmp_rsa_cli_ct_whole.bin";
    const char *pt2_path  = "res/test/runner/tmp_rsa_cli_pt_whole_dec.txt";
...
/* [TC2-1] 빈 파일 whole/raw 왕복 */
static void test_rsa_cli_whole_raw_empty(void) {
    const char *pt_path   = "res/test/runner/tmp_rsa_cli_pt_empty.txt";
    const char *ct_path   = "res/test/runner/tmp_rsa_cli_ct_empty.bin";
    const char *pt2_path  = "res/test/runner/tmp_rsa_cli_pt_empty_dec.txt";

    write_text_file(pt_path, ""); // create zero-byte file
...
/* [TC4-1] 잘못된 fmt 문자열("text") → 실패해야 함 */
static void test_rsa_cli_bad_format(void) {
    char *argv_bad[] = {
        "rsa_app",
        "enc",
        "whole",
...
static void test_rsa_cli_bad_key_hex_length(void) {
    const char *pt_path   = "res/test/runner/tmp_rsa_cli_badkey_pt.txt";
    const char *ct_path   = "res/test/runner/tmp_rsa_cli_badkey_ct.bin";
...
/* [TC6] whole/raw 복호 시 ciphertext 길이 불일치 → 실패해야 함 */
static void test_rsa_cli_bad_ct_length(void) {
    const char *ct_path   = "res/test/runner/tmp_rsa_cli_badct.bin";
    const char *pt2_path  = "res/test/runner/tmp_rsa_cli_badct_dec.txt";

    /* 2바이트 블록 키에 맞지 않는 1바이트 CT */
    uint8_t bad_ct[1] = { 0xAA };
    FILE *f = fopen(ct_path, "wb"); assert(f); fwrite(bad_ct,1,1,f); fclose(f);
...
int main(void) {
    ...
    test_rsa_cli_whole_raw_empty();
    test_rsa_cli_bad_argc();
    test_rsa_cli_bad_mode();
    test_rsa_cli_bad_format();
    test_rsa_cli_bad_key_hex_length();
    test_rsa_cli_bad_ct_length();
